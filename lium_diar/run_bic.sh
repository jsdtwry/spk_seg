...
${liumcmd} fr.lium.spkDiarization.programs.MSeg --kind=FULL --sMethod=$method --sModelWindowSize=$winshift --sMinimumWindowSize=$winshift --fInputMask=$fwav --fInputDesc=$fDesc --sInputMask=./$datadir/$show.i.seg --sOutputMask=./$datadir/$show.s.seg --sOutputFormat=eger.hyp $show
